...
tf.app.flags.DEFINE_integer("flexible_experts_num", 10, "Num of experts in the FEG.")
...
def model_fn(features, labels, mode, params):
    ...
    with tf.variable_scope("experts-part"):
        for j in range(FLAGS.flexible_experts_num + FLAGS.shared_experts_num):
    ...
    for expert_i in range(FLAGS.flexible_experts_num):
        for contrast_i in range(FLAGS.flexible_experts_num):
    ...
    final_outputs = []
    gate_networks_flexible, gate_networks_shared = [], []
    task_out_flexible, task_out_shared = [], []
    flexible_networks = []

    # FEG
    for i in range(FLAGS.task_num):
        gate_network_flexible = tf.contrib.layers.fully_connected(
            inputs=input_emb,
            num_outputs=FLAGS.flexible_experts_num,
            activation_fn=tf.nn.relu,
            weights_regularizer=l2_reg)
        gate_network_shape = gate_network_flexible.get_shape().as_list()
        gate_network_flexible = tf.nn.softmax(gate_network_flexible, axis=1)
        gate_network_flexible = tf.reshape(gate_network_flexible, shape=[-1, gate_network_shape[1], 1])
        gate_networks_flexible.append(gate_network_flexible)

        FEG_threshold = tf.layers.dense(inputs=similar_scene_emb,
                                        units=1,
                                        activation=tf.nn.sigmoid,
                                        kernel_initializer=tf.contrib.layers.xavier_initializer(),
                                        bias_initializer=tf.zeros_initializer())
        flexible_network = tf.layers.dense(inputs=tf.concat([similar_scene_emb, user_features], axis=-1),
                                           units=FLAGS.flexible_experts_num,
                                           activation=tf.nn.sigmoid,
                                           kernel_initializer=tf.contrib.layers.xavier_initializer(),
                                           bias_initializer=tf.zeros_initializer(),
                                           name='flexible_%d' % i)
        flexible_networks.append(dicaf(flexible_network, FEG_threshold, FLAGS.tau))

    for gate_network, flexible_network in zip(gate_networks_flexible, flexible_networks):
        combine_network = tf.multiply(gate_network, flexible_network)
        combine_network /= tf.reduce_sum(combine_network, axis=1, keepdims=True)
        task_out_ = tf.multiply(experts[:, 0: FLAGS.flexible_experts_num, :], combine_network)
        task_out_flexible.append(tf.reduce_sum(task_out_, axis=1))
    ...
    for i in range(FLAGS.task_num):
        final_outputs.append(tf.concat([task_out_flexible[i], task_out_shared[i]], axis=1))
    ...
    variance_loss = 0.
    for i in range(FLAGS.task_num):
        f_vec = tf.squeeze(flexible_networks[i], axis=-1)
        f_mean = tf.reduce_mean(f_vec, axis=-1, keepdims=True)
        top = tf.reduce_sum(tf.square(f_vec - f_mean), axis=-1)
        variance_loss -= tf.reduce_mean(top / FLAGS.flexible_experts_num, axis=0)
